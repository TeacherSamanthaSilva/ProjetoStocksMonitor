...
except Exception as e: # caso não exista, criar df
    print(f"⚠️ Nenhum book encontrado: {e}")
...
try:
    ...
    print(f"⚠️ Nenhum histórico encontrado: {e}")
    df_historical_data = pd.DataFrame(columns=['datetime', 'symbol', 'close'])

# Tratamento para evitar erro com None vindo do tvdatafeed
try:
    df_historical_data = atualizar_historical_data(df_historical_data, ativos_org)
except Exception as e:
    print(f"❌ Erro ao atualizar histórico inicial: {e}")
    df_historical_data = pd.DataFrame(columns=['datetime', 'symbol', 'close'])

# Converter para dict para usar no dcc.Store
...
def atualizar_databases(book_data, historical_data):
    ...
    try:
        ...
        print(f"❌ Erro ao atualizar dados históricos no callback: {e}")
